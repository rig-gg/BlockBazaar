...
function MyListings() {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      setListError("Cannot reach the server. Is the backend running?");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        setFormError("Could not list this item. Please try again.");
        return;
      }
      ...
      setFormError("Cannot reach the server. Is the backend running?");
    } finally {
      setSubmitting(false);
    }
  };
